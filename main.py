...
def get_args_from_parser() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="GR_MRL")
    ...
    parser.add_argument("--data_path", type=str, default="METR_LA", help="data file")
...
def update_args_from_fixed_params(
    args: argparse.Namespace, fixed_params: dict, logger: logging.Logger
) -> argparse.Namespace:
    # Update args
    for key, value in fixed_params.items():
        logger.info("### [Fixed] Set {} to {}".format(key, value))
...
def trainable(
    ...
    return_metrics["avg_mse"] = np.mean(
        [v["best_test_loss"] for v in metrics.values()]
    )
    return_metrics["avg_mae"] = np.mean(
        [v["best_test_mae"] for v in metrics.values()]
    )
    for pred_len in dft_args.pred_len_list:
        return_metrics[f"{pred_len}_mse"] = metrics[pred_len]["best_test_loss"]
        return_metrics[f"{pred_len}_mae"] = metrics[pred_len]["best_test_mae"]
...
if __name__ == '__main__':
    ...
    print(dataloaders[0].dataset[0].x.shape[-1])
